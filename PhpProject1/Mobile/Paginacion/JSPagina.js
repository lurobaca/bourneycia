...
// alert("Pag: "+ Pag+" LimitINICIO : " + LimitINICIO );
	 ...
	 ObtieneDatosDePagia(LimitINICIO,15,usuario,Busqueda,"NOMostrarRutero");
  ...
  ObtieneDatosDePagia(LimitINICIO,15,usuario,Busqueda,"NOMostrarRutero");
	...
	if (Pag >= MaxPaginasAMostrar)	{
		//permite marcar el primer cuadro cada ves que se optienen las paginas
		 PagActivoIni=PagActivoIni+5;
			     
       /*Ira a buscar los guientes 10 link de paginas*/
		ObtienePaginas(Pag,Pag,MaxPaginasAMostrar + 5,usuario,Busqueda,"Siguiente");
	}
	...
	// ObtieneDatosDePagia(LimitINICIO,25,usuario,Busqueda,"");
	 ...
	 if (Pag <= MaxPaginasAMostrar-6){
		//permite marcar el ultimo cuadro cada ves que se optienen las paginas
		  ...
		  /*Ira a buscar los guientes 10 link de paginas*/
		  ObtienePaginas(Pag,LiminIni,MaxPaginasAMostrar - 5,usuario,Busqueda,"Anterior");
		}
 	 ...
 	 //si no estan buscando ninguna descriocion , manda a buscar todos los articulos
					if (DESCRIPCION == ""  || DESCRIPCION == " " ){
						...
						if(BusquedaAnterior == DESCRIPCION)
							...
							if(LimiteINICIO == 0)
							ObtienePaginas(0,0,5,usuario," ","Siguiente");	
							...
							if(BusquedaAnterior == DESCRIPCION)
							   Pag == 0
						if(LimiteINICIO == 0)
						   ObtienePaginas(0,0,5,usuario,DESCRIPCION,"Siguiente")
						}
...
function BuscaOfertas(LimiteINICIO,LimiteFIN,usuario){	
		...
		var DESCRIPCION = "of "
		$('#tags1').val("of ");
		ObtienePaginas(1,5,usuario,DESCRIPCION,"Siguiente") 
		
		if (DESCRIPCION == "")
		{
		ObtieneDatosDePagia(0,15,usuario);
        ObtienePaginas(1,5,usuario,"","Siguiente");
		}else
		{
     	...
     	$.ajax({
			...
			beforeSend: function () { 
							    ...
							    document.getElementById('fade').style.display='block';
		  						//document.getElementById('fade').html("Procesando, espere por favor...");

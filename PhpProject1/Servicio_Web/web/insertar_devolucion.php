...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Insertar gasto
    $Existe = Devoluciones::ExisteLineaDevolucion($body);
    ...
    if (strcmp(trim($Existe), "NO EXISTE") == 0){//si no existe la linea la manda a insertar 
        ...
        $idGasto = Devoluciones::InsertDevolucion($body);
        ...
        if ($idGasto) {
            ...
            print json_encode(
                array(
                    ESTADO => CODIGO_FALLO,
                    MENSAJE => 'Creación fallida //'.$pdo->lastInsertId())
            );
        }
    }else{//Si existe se debe actualizar o eliminar la linea y volverla a ingresar
        if(trim($Existe)=="0") {
            Devoluciones::ActualizaDevolucion($body);          
        }else{
              print json_encode(
                array(
                    ESTADO => CODIGO_FALLO,
                    MENSAJE => 'El pedido ya esta en SAP los cambios aplicados deben crearlos en un nuevo pedido')
            ); 
        }

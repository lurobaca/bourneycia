...
class Pedidos
{
	...
	*/
	//------------------- TABLA PEDIDOS -----------------------
		const TABLE_PEDIDOS = "Seller_Pedido";
    ...
    /*    //------------------- TABLA RECIBO -----------------------
        ...
        //------------------- TABLA RECIBO -----------------------
        const TABLE_DEPOSITOS = "depositos";
...
*/
	...
	public static function Obtiene_Un_Pedidos($DocNumUne,$ItemCode ,$Porc_Desc )
	{
    ...
    public static function getAll_Pedidos()
    {

        $consulta = "SELECT * FROM " . self::TABLE_PEDIDOS;
        try {
            // Preparar sentencia
            $comando = DatabaseConnection::getInstance()->getDb()->prepare($consulta);
            // Ejecutar sentencia preparada
 ...
 public static function ExisteLinea($object){
    try {   
        $Retorno  ='';
        ...
        {  
            if($object['idRemota']!='0'){ //si lo que tiene es diferente de 0
                ...
                $consulta ="SELECT * FROM ". self::TABLE_PEDIDOS . " WHERE idPedido ='" . $object["idRemota"] . "'";
...
public static function InsertPedido($object){
        try {
             $pdo = DatabaseConnection::getInstance()->getDb();
                  // ------------Sentencia INSERT --------------------
            $comando = "INSERT INTO " . self::TABLE_PEDIDOS . " ( " .
                self::DocNumUne . "," .
                self::DocNum . "," .
                self::CodCliente . "," .
                self::Nombre . "," .
            ...
            $sentencia->bindParam(20, $Hora_Pedido);
      ...
      //-----------DECODIFICA JSON --------------------------
            ...
            $Hora_Pedido = $object[self::Hora_Pedido];
...
public static function ActualizaPedido($object){
 try {  
        $pdo = DatabaseConnection::getInstance()->getDb();
        // ------------Sentencia INSERT --------------------
        $comando = "UPDATE " . self::TABLE_PEDIDOS . " SET " .
            self::DocNumUne . "=?," .
            self::DocNum . "=?," .
            self::CodCliente . "=?," .
            self::Nombre . "=?," .
            ...
            $sentencia->bindParam(27, $Anulado);
            $sentencia->bindParam(28, $EnSAP);
            $sentencia->bindParam(29, $idPedido);
...
public static function EliminaPedido($object){
 try {  
        $pdo = DatabaseConnection::getInstance()->getDb();
        // ------------Sentencia INSERT --------------------
        $comando = "DELETE FROM " . self::TABLE_PEDIDOS . " WHERE ". self::idPedido ."=?" ;   
...
public static function AnulaPedido($object){
 try {  
        $pdo = DatabaseConnection::getInstance()->getDb();
        // ------------Sentencia INSERT --------------------
        $comando = "UPDATE " . self::TABLE_PEDIDOS . " SET Anulado='1' WHERE ". self::idPedido ."=?" ;   

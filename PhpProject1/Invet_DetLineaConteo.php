...
<html lang="es">
	<head>
	<style>
body {background-color:#0033FF; color: #FFFFFF;}
	...
	<body>
		...
		<form action="#" method="post" class="formul" style=" background-color:#0033FF; color: #FFFFFF;  ">
			...
			<input name="nombre" id="CodArticulo" type="text" class="CajaTexto" placeholder=""   value="<?php echo $_GET["CodArticulo"]; ?>" readonly /><br />
				<LABEL>Cod Barras</LABEL>
			<input name="nombre" id="CodBarras" type="text" class="CajaTexto" placeholder=""   value="<?php echo $Articulos['CodBarras'];   ?>" readonly /><br />
			...
			<input name="nombre" id="Contaron" type="number" class="CajaTexto" placeholder="Cantidad aqui"   value="<?php if($Articulos['Contaron']=="0") {echo "";} else echo $Articulos['Contaron'];      ?>"  autocomplete="off" /><br />

			<div style="width:100%;  overflow: auto;">	
		
			<DIV style="float: left; ">
				<button name="btsend" id="btsendContact" type="submit" class="myButton" onclick="Conto($('#CodArticulo').val(),$('#Contaron').val(),$('#Apuntes').val(),$('#Conteo').val(),$('#Costo').val(),$('#Stock').val(),$('#Busqueda').val(),'<?php echo $_GET['Grupo'];?>','<?php echo $_GET['Conteo']?>','<?php echo $_GET['CodProveedor']?>'); return false">ACEPTAR</button>
				</div>
                	<DIV style="float: left; ">
				
				<button name="btsend" id="btRegresar" type="submit" class="myButton" onclick="Regresar('Lista',$('#Busqueda').val(),'<?php echo $_GET['Grupo'].$_GET['Conteo'];?>','<?php echo $_GET['Grupo'];?>','<?php echo $_GET['Conteo'];?>','<?php echo $_GET['CodProveedor'];?>'); return false">REGRESAR</button>

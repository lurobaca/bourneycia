    ...
    $.fn.jSlider = function(options) {
        ...
        return this.each(function() {
			...
			function createPagination(curr) {
				...
				var start, items = "", nav = "";
                start = "<ul id='navi' class='"+options.navi_class+"'>";
                var end = "</ul>"
				for (i=1;i<=number_of_items;i++)
                {
					if (i == curr) { items += '<li class="ItemNav"><a class="'+options.navi_active_class+'" title="'+i+'">'+i+'</a></li>';} 
					else { items += '<li class="ItemNav"><a href="#" class="goto" title="'+i+'">'+i+'</a></li>';}
                }
                nav = start + items + end;
				obj.append(nav);
            }
